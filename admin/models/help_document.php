...
/**
 * Demo Help_document Model
 */
class DemoModelHelp_document extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_demo.help_document';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'help_document', $prefix = 'DemoTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_demo.help_document');
			}
		}
	...
	{		// Get the form.
		...
		if ($id != 0 && (!$user->authorise('help_document.edit.state', 'com_demo.help_document.' . (int) $id))
			|| ($id == 0 && !$user->authorise('help_document.edit.state', 'com_demo')))
		{
	...
	{
		return 'administrator/components/com_demo/models/forms/help_document.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('help_document.delete', 'com_demo.help_document.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('help_document.edit.state', 'com_demo.help_document.' . (int) $recordId);
		...
		return $user->authorise('help_document.edit.state', 'com_demo');
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__demo_help_document'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null)
			{
				...
				{
					$msg = JText::_('COM_DEMO_HELP_DOCUMENT_SAVE_WARNING');
				}
